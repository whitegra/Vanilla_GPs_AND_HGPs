...
def train_heteroskedastic_gp_model(X, y, noise_variance):
    ...
    def custom_predict(X_new):
        ...
        # Adding per-data-point variance
...
def load_data_from_csv(file_path):
    """Load X (x1, x2) and y from a CSV file."""
...
noise_model_y1 = train_noise_model(X_noisy, y1_noisy)
pred_noise_var_y1, _ = noise_model_y1.predict_f(X_noisy)
pred_noise_var_y1 = pred_noise_var_y1.numpy()

# Use the custom prediction method
f_mean_y1, f_var_y1 = train_heteroskedastic_gp_model.custom_predict(X_plot)

# Train noise model for y2
noise_model_y2 = train_noise_model(X_noisy, y2_noisy)
pred_noise_var_y2, _ = noise_model_y2.predict_f(X_noisy)
pred_noise_var_y2 = pred_noise_var_y2.numpy()

# Train heteroskedastic GP model for y2
heteroskedastic_gp_model_y2 = train_heteroskedastic_gp_model(X_noisy, y2_noisy, pred_noise_var_y2)
...
heteroskedastic_gp_model_y1 = train_heteroskedastic_gp_model(X_noisy, y1_noisy, pred_noise_var_y1)
...
# Generate grid for prediction (assuming x_limits are the same for both inputs)
X_plot = np.array(np.meshgrid(np.linspace(x_limits[0], x_limits[1], 100),
                              np.linspace(x_limits[0], x_limits[1], 100))).T.reshape(-1, 2)

# Predict f_mean and f_var for the models
f_mean_y1, f_var_y1 = heteroskedastic_gp_model_y1.predict_f(X_plot)
f_mean_y1 = f_mean_y1.numpy()
f_var_y1 = f_var_y1.numpy()

f_mean_y2, f_var_y2 = heteroskedastic_gp_model_y2.predict_f(X_plot)
f_mean_y2 = f_mean_y2.numpy()
f_var_y2 = f_var_y2.numpy()
...
y_best_f1 = np.min(y1_noisy)
y_best_f2 = np.min(y2_noisy)
